require("core.plugin_config.dap")
require("core.plugin_config.gp")
require("core.plugin_config.lsp")
require("core.plugin_config.mini.comment")
require("core.plugin_config.mini.completion")
require("core.plugin_config.mini.files")
require("core.plugin_config.mini.jump")
require("core.plugin_config.mini.jump2d")
require("core.plugin_config.mini.pick")
require("core.plugin_config.obsidian")
require("core.plugin_config.snippy")
require("core.plugin_config.toggle-term")
require("core.plugin_config.treesitter")
require("core.plugin_config.undotree")
